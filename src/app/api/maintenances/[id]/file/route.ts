...
export async function GET(_request: NextRequest, { params }: RouteContext) {
  ...
  if (!maintenance.filePath || !maintenance.originalFileName) {
    return NextResponse.json({ error: 'PDF non ancora caricato' }, { status: 404 });
  }

  try {
    const { fileBuffer, fileStat } = await readStoredPdf(maintenance.filePath);
    return new NextResponse(fileBuffer, {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Length': String(fileStat.size),
        'Content-Disposition': contentDisposition(maintenance.originalFileName),
        'X-Content-Type-Options': 'nosniff',
        'Cache-Control': 'private, no-store'
      }
    });
  } catch (error) {
    console.error('Unable to read stored maintenance PDF', { maintenanceId: maintenance.id, error });
